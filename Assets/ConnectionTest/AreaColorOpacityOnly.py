...
if __name__ == '__main__':
    ...
    for image_path, imageAndLabel_path, mask_path in zip(args.image_paths, args.imageAndLabel_paths, args.mask_paths):
        ...
        imageFlat = backgroundImgAsTensor.view(1,3,-1) #[1,3,numPixels] # only 3 channels so need to add alpha channel
        ...
        # labelFlat2.fill_(-1.0) # fill with all black values
         ...
         # Add opacity as a trainable variable
        opacityVar = Variable(torch.rand(1, numLabelPixels), requires_grad=True)
        ...
        optimizer = torch.optim.Adam([labelVar, opacityVar], lr=args.lr, betas=(0.9, 0.999))
        ...
        for iter in range(MAX_ITER): 
            ...
            # Update label pixels with opacity
            labelVarRGBA = torch.cat((labelVarFlat, opacityVar.view(1, -1)), dim=1)
            full_img.index_put_(labelIndices, labelVarRGBA.view(1, -1).reshape(labelVarRGBA.size()[1]))
            
            full_img_flat = full_img # still flat: [1, 3, numPixels] (after including Alpha channel: [1,4,numPixels])
            full_img = full_img.view(1,4,height,width) # restore its shape to match the original image's shape : [1, 3, width, height]
            ...
            if (iter == MAX_ITER - 1): 
                ...
                full_img = full_img.view(1,4,height,width) # restore its shape to match the original image's shape
                full_img.data = torch.clamp(full_img.data, -1, 1)

                # Save the final result
                pred_img = lpips.tensor2im(full_img.data, rgba=True)
                image_name = os.path.splitext(os.path.basename(image_path))[0]  # Extract the base name without the extension

                if args.metric == 'ssim':
                    output_path = f"./test20240201/{image_name}_weight-{weight}_{args.metric}_a-{alpha}b-{beta}c-{gamma}_itr{args.itr}_lr{args.lr}_deltaE-{args.deltaE}.jpg"
                    ...
                    output_path = f"./test20240201/{image_name}_weight-{weight}_{args.metric}_itr{args.itr}_lr{args.lr}_deltaE-{args.deltaE}.jpg"
