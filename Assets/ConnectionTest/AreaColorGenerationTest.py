    ...
    for image_path, imageAndLabel_path, mask_path in zip(args.image_paths, args.imageAndLabel_paths, args.mask_paths):
        ...
        for iter in range(MAX_ITER): 
            ...
            if (iter == MAX_ITER - 1): 
                ...
                image_name = os.path.splitext(os.path.basename(image_path))[0]  # Extract the base name without the extension
                if args.blur:
                    output_path = f"./testResults/test20231205_ssim_exponents/{image_name}_weight-{weight}_{args.metric}_blurredBG_sigma{args.sigma}_itr{args.itr}_lr{args.lr}_deltaE-{args.deltaE}.jpg"
                else:
                    if args.metric == 'ssim':
                        output_path = f"./testResults/test20231205_ssim_exponents/{image_name}_weight-{weight}_{args.metric}_a-{alpha}b-{beta}c-{gamma}_unblurredBG_itr{args.itr}_lr{args.lr}_deltaE-{args.deltaE}.jpg"
                        ...
                        output_path = f"./testResults/test20231205_ssim_exponents/{image_name}_weight-{weight}_{args.metric}_unblurredBG_itr{args.itr}_lr{args.lr}_deltaE-{args.deltaE}.jpg"
